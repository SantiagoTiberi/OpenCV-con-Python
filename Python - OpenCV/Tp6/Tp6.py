import cv2 as cv
import numpy as np
from similariadad import simil


save = False
ix, iy, ix2, iy2 = 0 ,0, 0, 0
#angle, tx, ty, escala = 45, 100, 100, 1.8
angle=int(input('Ingrese angulo:'))
tx = int(input('Ingrese traslacion en X:'))
ty = int(input('Ingrese traslacion en Y:'))
escala = float(input('Ingrese escala:'))
drawing = False
...
def save_img(ix, iy, ix2, iy2, img):
    ...
    #cut = np.zeros((ry+100,rx+100,3), np.uint8) #numpy arrays, creo una matriz de tamaño ry y rx (3D por los colores) para guardar la imagen
...
while(1):
    ...
    if k == ord('r'):
        ...
        iy, iy2 = min(iy, iy2), max(iy, iy2)  #lo mismo en la varbiale Y 
        cut = roi[iy:iy2, ix:ix2]
        cut = simil(cut, angle, tx, ty, escala)
        cv.imwrite('resultado similaridad.png', cut)
        cv.imshow('recorte', cut)
        i=1
